...
void RootFrame::imageUploadHandler(wxCommandEvent &evt)
{
    ...
    if (openFileDialog.ShowModal() == wxID_OK)
    {
        ...
        if (bitmap.LoadFile(imgPlayBack->getImageByIndex(imgPlayBack->getCurrentIndex()), wxBITMAP_TYPE_ANY))
        {
            ...
            this->SetTitle("GloApp | " + std::string(wxFileName(imgPlayBack->getImageByIndex(imgPlayBack->getCurrentIndex())).GetFullName().mb_str()));
        }
    }
}
...
void RootFrame::updateVideoViewer(wxString videoFilePath)
{
    ...
    this->SetTitle("GloApp | Playing " + std::string(wxFileName(videoFilePath).GetFullName().mb_str()));
    isPlaying = false;
    volume = 1.0;
...
void RootFrame::previousImageHandler(wxCommandEvent &evt)
{
    ...
    if (tempCurrentIndex != 0)
    {
        ...
        if (bitmap.LoadFile(imgPlayBack->getImageByIndex(tempCurrentIndex - 1), wxBITMAP_TYPE_ANY))
        {
            ...
            if(!slideShowFlag)
            {
                this->SetTitle("GloApp | " + std::string(wxFileName(imgPlayBack->getImageByIndex(tempCurrentIndex - 1)).GetFullName().mb_str()));
            }
        }
    }
}
...
void RootFrame::slideshowOpenClose(wxCommandEvent &evt)
{
    ...
    if (slideShowFlag)
    {
        ...
        this->SetTitle("GloApp | Image Slideshow");
    }
    else
    {
        slideShowTimer.Stop();
